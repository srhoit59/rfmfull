...
byte_stream = BytesIO()
blobservice.get_blob_to_stream(container_name='htflaskcontainer', blob_name='totalsdu.xlsx', stream=byte_stream)
byte_stream.seek(0)
ser=pd.read_excel(byte_stream,index_col=0)
byte_stream.close()
#ser.head()
#ser = pd.read_excel('Copy of DB-O.xlsx',sheet_name='TotDB1-O',index_col=0)
for i in range(0,4):
...
#print(adrf)
#my_model.plot(forecast,uncertainty=True)
b= pd.DataFrame(forecast[['ds','yhat']])
...
blobservice = BlockBlobService(account_name='flaskstorage', account_key='M9Hax/c6wKCdVXIcmBafad35/ctWW2OQJQynRMrM29D+mfZXWW53MF0Sthsf0cmWN+/XukVg/aZQ/6XBAB4cgg==') 
data= BytesIO()
newdf.to_excel(data, index=False)
data=bytes(data.getvalue())
data=BytesIO(data)
blobservice.create_blob_from_stream('attritioncontainer','attrition.xlsx',data)
data.close()
