...
byte_stream = BytesIO()
blobservice.get_blob_to_stream(container_name='htflaskcontainer', blob_name='eegdu.xlsx', stream=byte_stream)
byte_stream.seek(0)
ser=pd.read_excel(byte_stream,index_col=0)
byte_stream.close()
#ser.head()
#ser = pd.read_excel('Copy of DB-O.xlsx',sheet_name='EEGDB1-O',index_col=0)
for i in range(0,4):
...
data=BytesIO(data)
blobservice.create_blob_from_stream('eegcontainer','EEGINC.xlsx',data)
data.close()
"""
from pandas import ExcelWriter
if not os.path.exists('Generated-Excels'):
    os.makedirs('Generated-Excels')
writer = ExcelWriter('Generated-Excels/EEGINCR.xlsx')
