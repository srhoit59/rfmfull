...
byte_stream = BytesIO()
blobservice.get_blob_to_stream(container_name='htflaskcontainer', blob_name='mobiledu.xlsx', stream=byte_stream)
byte_stream.seek(0)
ser=pd.read_excel(byte_stream,index_col=0)
byte_stream.close()
ser.head()
#ser = pd.read_excel('Copy of DB-O.xlsx',sheet_name='MOBILEDB1-O',index_col=0)
#for i in range(0,4):
